...
class TestTodo : StringSpec({
    ...
    "할일이 처음 생성되면 상태는 'TODO'여야 한다." {
        ...
        val description = "테스트코드를 처음 작성해봅니다."
        val item = todoService.createTodo(title, description)
        item.status shouldBe TodoStatus.TODO
    }
    ...
    "할일을 완료 상태로 변경할 수 있다." {
        ...
        item.status shouldBe TodoStatus.DONE
    }
    ...
    "TODO 상태의 할일을 DONE 으로 변경할 수 있다." {
        ...
        val changedItem = todoService.changeStatus(item, TodoStatus.DONE)
        changedItem.status shouldBe TodoStatus.DONE
    }
    ...
    "할일 목록을 상태로 필터링할 수 있다." {
        ...
        todoService.changeStatus(item3, TodoStatus.DONE)

        val todoStatusItems = todoService.getTodos(status = TodoStatus.TODO)
        todoStatusItems.size shouldBe 2

        val doneStatusItems = todoService.getTodos(status = TodoStatus.DONE)
    ...
    "할일 상태가 DONE 으로 변경되면 완료 시간이 기록된다." {
        ...
        val description = "테스트코드를 처음 작성해봅니다."
        val item = todoService.createTodo(title, description)
        item.doneAt shouldBe null

        todoService.changeStatus(item, TodoStatus.DONE)
        item.doneAt shouldNotBe null
    }
})